...
- **Behavior changed:** Parent evidence gathering changed none. The approved PDF child surfaces marker-only extraction; the approved plain-text child surfaces invalid-UTF-8 replacement and NUL removal through existing warnings without changing normalized content or schema.
...
- **Blocker:** `NONE`; the evidence slice is complete, and both narrow existing-warning-field child fixes were independently approved and closed without selecting the owner-gated semantic schema. Broader normalization/capability architecture remains owner-gated.
- **Unblock owner:** `NONE`
