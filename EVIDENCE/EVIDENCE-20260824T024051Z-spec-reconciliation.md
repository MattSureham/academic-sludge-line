...
| `2026-08-24T03:31:59Z` | `agent:codex-governance-review` | Corrected §4.4 Revision guidance from **partially implemented** to **implemented**. | Structured prompt sections, separately persisted reviewer reports, synthesized actionable revision guidance, next-iteration reuse, and parsed underused-reference focus satisfy the mandatory behavior. A validated schema and per-persona route are optional enrichment, not current gaps; see [independent review evidence](EVIDENCE-20260824T033159Z-governance-independent-review.md). |
| `2026-08-24T03:31:59Z` | `agent:codex-governance-review` | Corrected §4.10 Capability-aware execution from **absent** to **partially implemented**. | `ModelPreset` and `LocalModelPreset` carry descriptive capability tuples and `catalog_payload()` exposes them, but stages and route execution do not declare or negotiate capability requirements; see [independent review evidence](EVIDENCE-20260824T033159Z-governance-independent-review.md). |
| `2026-08-24T07:32:44Z` | `agent:codex-normalization-evidence` | Corrected §11 invariant 13 from **implemented** to **partially implemented**. | The original recovery trace established warning fields and several PDF/CAJ degradation paths, but the representative fixture matrix demonstrates silent known loss for an image-only PDF, invalid UTF-8/NUL text, duplicate-key JSON, ragged CSV, malformed HTML, and directory-ignored unsupported files. See [normalization fixture evidence](EVIDENCE-20260824T073244Z-normalization-fixture-matrix.md). |
