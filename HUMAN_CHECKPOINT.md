...
- **Classification:** Governance/recovery, score validation, and page-marker-only warning baselines are `APPROVED`; the invalid-UTF-8/NUL warning candidate is in independent `REVIEW`; normalization remains `PARTIALLY IMPLEMENTED` because other observed fidelity gaps remain; product architecture is otherwise `UNKNOWN` beyond recorded traces
...
| `CONFIRMED` | Invalid UTF-8 replacement and NUL removal were silent in the plain-text fixture | The focused candidate emits one attributable warning per observed source-byte condition while retaining identical normalized text/Markdown/chunks; valid UTF-8 and all other curated documents are unchanged | [Invalid-UTF-8/NUL evidence](EVIDENCE/EVIDENCE-20260825T024729Z-invalid-utf8-nul-warning.md); independent review pending |
...
- Normalization fixture gathering is complete. The bounded page-marker-only PDF slice is independently approved and closed. The invalid-UTF-8/NUL candidate likewise reuses existing warnings under accepted §§2.5/4.8 without selecting new architecture; fresh independent review is required, but no owner decision is needed. Any canonical normalization schema, fidelity threshold, broader parser strategy, or capability-negotiation contract remains owner-gated.

## Next checkpoint trigger

- **Trigger:** Fresh independent disposition on the invalid-UTF-8/NUL warning candidate, any owner response above, or a proposed normalization/evidence/capability ADR.
- **Expected owner action before then:** No action is required for score validation, the closed PDF-warning slice, or the pending focused review. Decide only `DECISION-ACCEPTED-POINTER-RECOVERY-v1` if pointer semantics are to enter implementation; other decisions may wait until their issues become selected work.
